...
class Log
{
    ...
    //日志配置
    static protected $logConfig = array();
    ...
    {
        self::$logConfig = \Yaf\Registry::get('log');
        if ($record || false !== strpos(self::$logConfig['level'], $level)) {
    ...
    static function save($type = '', $destination = '')
    {
        ...
        if (empty($destination)) {
            $destination = C('LOG_PATH') . DS . date('y_m_d') . '.log';
        }
        if (!self::$storage) {
            $type          = $type ?: self::$logConfig['type'];
