...
// 	window.editor = K.create('#editBox', {
...
// 		],
// 		uploadJson: $.ROOT.apiurl + 'Common/upload'
// 	});
		...
		$.ROOT.request(api, param, function (data) {
			...
			if(data.status == 1){
				var i = layer.confirm("修改成功！", {
					...
					_thisObj.hide();
					_thisIframe.hide();
					_parent.find(".J_menuTab").each(function () {
						var t = $(this).data("id");
...
function getInfo() {
	...
	$.ROOT.request("Content/getInfo", param, function (data) {
		if(data.status == 1){
			if(data.info.is_jump == 0){
				$(".art-content").show();
				// ue.addListener("ready", function () {
				// 	// editor准备好之后才可以使用
				// 	this.setContent(data.info.content);

				// });
				$("#editBox").html(data.info.content);
				ue = UE.getEditor('editBox'); // Ueditor 编辑器
			}
			var gettpl = document.getElementById('boxTpl').innerHTML;
			laytpl(gettpl).render(data, function(html){
			    document.getElementById('box').innerHTML = html;
			    var L = data.info.pids.split(",");
			    getBrandList(L);
